...
- Default options now follow SvelteKit more closely: `resetForm: true` and `taintedMessage: false` are defaults now. Add `define: { SUPERFORMS_LEGACY: true }` in `vite.config.ts` to keep the old behavior.
...
- Superform validator functions requires the `superformClient` adapter, and its input can now be `undefined`.
...
- Added `superForm.isTainted(path?)` and `superForm.isTainted($tainted)` for better tainted check.
...
- All proxies can now take the whole `superForm` object, with an extra `taint` option to prevent tainting.
