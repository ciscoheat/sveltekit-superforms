...
import { SuperFormError, type InputConstraint, type UnwrapEffects } from '../index.js';
...
>(form: Writable<T>, path: Path, type: Type, options: DefaultOptions): Writable<string> {
	function toValue(val: unknown) {
		...
		if (!val && options.empty !== undefined) return options.empty === 'null' ? null : undefined;
...
>(
	...
	const constraintsPath = (path2 as unknown[]).filter((p) => isNaN(parseInt(String(p)))).join('.');

	return {
		path,
		value: _stringProxy(form.form, path, type, options),
		errors: fieldProxy(form.errors, path) as unknown as Writable<string[] | undefined>,
		constraints: fieldProxy(form.constraints, constraintsPath as never) as Writable<
