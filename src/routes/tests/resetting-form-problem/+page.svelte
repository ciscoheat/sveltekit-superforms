...
<div class="flex justify-center items-center h-full w-full">
  ...
  <form method="POST" use:enhance>
    <div class="block card p-4 w-screen max-w-xl">
      ...
      />
      {#if $errors.email}<small class="text-red-500"
          >E-mail must be
        </small>{/if}
      ...
      />
      {#if $form.password !== $form.confirmPassword}<small
      ...
      />
      {#if $errors.name}<small class="text-red-500">{$errors.name}</small
        >{/if}
      {#if $errors.email}<small class="text-red-500">{$errors.email}</small
        >{/if}

      {#if $errors.password}<small class="text-red-500"
